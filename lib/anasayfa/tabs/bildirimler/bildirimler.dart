...
class _BildirimlerState extends State<Bildirimler> {
  ...
  Future _arkIstekReddet(Bildirim bildirim) async {
    ...
    await Firestore.instance.collection('uyeler').document(Fonksiyon.uye.uid).get().then((onValue) {
      ...
      Firestore.instance.collection('bildirimler').document(bildirim.id).delete();
    });
    _bildirimCek();
  }

  Future _sil(Bildirim bildirim) async {
    await Firestore.instance.collection('bildirimler').document(bildirim.id).delete();
  ...
  Future _arkIstekOnayla(Bildirim bildirim) async {
    ...
    await Firestore.instance.collection('uyeler').document(Fonksiyon.uye.uid).get().then((onValue) {
      ...
      Firestore.instance.collection('uyeler').document(onValue.documentID).updateData({'arkadaslar': e});
      Fonksiyon.uye.arkadaslar = e;
      Firestore.instance.collection('bildirimler').document(bildirim.id).delete();
      Firestore.instance.collection('uyeler').document(bildirim.gonderen).get().then((onValue) {
        List s = [];
        s = onValue.data['arkadaslar'] ?? [];
        s.add(bildirim.alici);
        Firestore.instance.collection('uyeler').document(onValue.documentID).updateData({'arkadaslar': s});
  ...
  void initState() {
    ...
    _scrollController.addListener(() {
      ...
      if (_sO + 220 > _sY && !_islem) _bildirimCek();
    });
                  ...
                  Column(
                    ...
                    children: <Widget>[
                      for (int i = 0; i < _bildirimler.length; i++)
                        if (_bildirimler[i].gonderen != Fonksiyon.uye.uid)
                                ...
                                children: <Widget>[
                                  Container(
                                    ...
                                    decoration: BoxDecoration(
                                      ...
                                      image: DecorationImage(image: NetworkImage(_uyeler[i].resim), fit: BoxFit.cover),
                                    ),
                                  ),
                                  Column(
                                    ...
                                    children: <Widget>[
                                      ...
                                      Container(
                                        child: Text('Sana arkadaşlık istegi gönderdi'),
                                      )
                                    ],
                                  ),
                                  SizedBox(
                              ...
                              child: Row(
                                children: <Widget>[
                                  ...
                                  Expanded(
                                    child: Column(
                                      ...
                                      children: <Widget>[
                                        ...
                                        Container(
                                          child: Text('Mesaj: ${_bildirimler[i].aciklama}'),
                                        )
                                      ],
                                    ),
                                  ),
                                  InkWell(
                                      ...
                                      child: Container(
                                        decoration:
                                            BoxDecoration(color: Renk.gGri19, borderRadius: BorderRadius.circular(50)),
                                        child: Icon(
                                          Icons.close,
                                          color: Renk.beyaz,
                                        ),
                              ...
                              child: Row(
                                children: <Widget>[
                                  Container(
                                    ...
                                    margin: EdgeInsets.only(right: 5),
                                    decoration: BoxDecoration(
                                      border: Border.all(color: Renk.gGri, width: 1),
                                      borderRadius: BorderRadius.circular(50),
                                      image: DecorationImage(image: NetworkImage(_uyeler[i].resim), fit: BoxFit.cover),
                                    ),
                                  ),
                                    ...
                                    child: Row(
                                      ...
                                      children: <Widget>[
                                        FlatButton(
                                            ...
                                            shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(30)),
                                            color: Renk.wpAcik,
                                            child: Text('Profili gör', style: TextStyle(color: Renk.beyaz)),
                                            onPressed: () {
                                              Navigator.push(
                                                  context,
                                                  MaterialPageRoute(
                                        ...
                                        InkWell(
                                            ...
                                            child: Container(
                                              decoration: BoxDecoration(
                                                  color: Renk.gGri19, borderRadius: BorderRadius.circular(50)),
                                              child: Icon(
                                                Icons.close,
                                                color: Renk.beyaz,
                                              ),
                          ...
                          else if (_bildirimler[i].tur == 'veri_tepki')
                            Container(
                              margin: EdgeInsets.symmetric(vertical: 2),
                              padding: EdgeInsets.symmetric(vertical: 8, horizontal: 8),
                              color: Renk.beyaz,
