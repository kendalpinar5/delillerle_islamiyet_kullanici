...
class SozWidget extends StatefulWidget {
  ...
  const SozWidget({Key key, @required this.soz, this.scaffoldKey}) : super(key: key);
  @override
...
class _SozWidgetState extends State<SozWidget> {
  ...
  Future sozuGuncelle() async {
    await _db.collection('gunun_sozleri').document(_soz.id).updateData(_soz.toJson());
    setState(() {});
  }
  ...
  void initState() {
    ...
    if (_soz.begenme.contains(Fonksiyon.uye.uid)) Fonksiyon.begenenSozler.add(_soz.id);
    if (_soz.begenmeme.contains(Fonksiyon.uye.uid)) Fonksiyon.begenmeyenSozler.add(_soz.id);
